...
class _AdminDashboardState extends State<AdminDashboard> {
  ...
  void initState() {
    _restoreSignin();
    userInfo.writeIfNull("allDataLogs", []);
    userInfo.writeIfNull("allComplaints", []);
    userInfo.writeIfNull("allRefunds", []);
    userInfo.write("allRefunds", []);
    userInfo.writeIfNull("allAccounts", []);
    userInfo.writeIfNull("allOrders", []);
  ...
  _fetchRequiredData() async {
    try {
      ...
      if (packages.data["success"]) {
        setState(() {
          userInfo.write("allPackages", packages.data["data"]);
        });
        chartmodels.fetchPackageCategories();
      }
      if (accounts.data["success"]) {
        setState(() {
          userInfo.write("allAccounts", accounts.data["data"]);
        });
        chartmodels.fetchAccountCategories();
      }
      if (orders.data["success"]) {
        setState(() {
          userInfo.write("allOrders", orders.data["data"]);
          currentMonth =
              userInfo.read("allOrders")[0]["dateplaced"].split("-")[1];
        });
      ...
      if (refunds.data["success"]) {
        setState(() {
          userInfo.write("allRefunds", refunds.data["data"]);
        });
        chartmodels.fetchRefundCategories();
      }
      if (logs.data["success"]) {
        setState(() {
          userInfo.write("allDataLogs", logs.data["data"]);
        });
      ...
      try {
        ...
        if (response1.data["success"]) {
          var response2 = await dio.post(
            "$backendUrl2/restore-signin",
            data: {"email": response1.data["data"]["user_id"]},
          );
          setState(() {
            userInfo.write("adminLoggedIn", true);
