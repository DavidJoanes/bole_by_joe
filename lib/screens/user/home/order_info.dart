...
class _OrderInfoState extends State<OrderInfo> {
  ...
  fetchItemsOrdered() {
    userInfo.read("itemsOrdered").clear();
    setState(() {
      for (var item in userInfo.read("activeDeliveryRequests")) {
        for (var item2 in item["items"] as List) {
          if (widget.orderId == item["orderid"]) {
            userInfo.read("itemsOrdered").add(item2);
          }
        }
      }
      // print("items ordered = ${userInfo.read("itemsOrdered")}");
    });
  }

  @override
  void initState() {
    userInfo.writeIfNull("orderOwnerData", {});
    userInfo.writeIfNull("itemsOrdered", []);
  ...
  Widget mixedScreen(BuildContext context) {
    ...
    final fontStyle1b =
        GoogleFonts.poppins(textStyle: const TextStyle(fontWeight: FontWeight.w500));
              ...
              children: [
                ...
                SizedBox(
                  ...
                  child: ListView.builder(
                      itemCount: userInfo.read("itemsOrdered").length,
                          ...
                          child: Card(
                            ...
                            child: ListTile(
                              leading: ProfilePicture(
                                image: userInfo.read("itemsOrdered")[index]
                                    ["coverimage"]["url"],
                                radius: 20,
                                onClicked: () {},
                              ),
                              title: Text(userInfo.read("itemsOrdered")[index]
                                  ["packagename"]),
                              subtitle: Text(
                                  "Qty: ${userInfo.read("itemsOrdered")[index]['qty']}"),
                              trailing: Text(
                                  "${currencyIcon(context).currencySymbol}${userInfo.read("itemsOrdered")[index]['price']}"),
                            ),
                          ),
                        );
                      }),
          ...
          OverflowBar(
            children: [
              Container(
                ...
                child: ListTile(
                  title: const Text("Estimated delivery duration (in days)"),
                  subtitle: Card(
                    child: Padding(
                      padding:
              ...
              Container(
                ...
                child: ListTile(
                  title: const Text("Estimated delivery duration (in minutes)"),
                  subtitle: Card(
                    child: Padding(
                      padding:
                          EdgeInsets.symmetric(horizontal: size.width * 0.02),
  ...
  _fetchOrderOwnerData() async {
    setState(() {
      userInfo.read("orderOwnerData").clear();
    });
    ...
    try {
      ...
      if (response.data["success"]) {
        setState(() {
          userInfo.write("orderOwnerData", response.data["data"]);
          userImage = userInfo.read("orderOwnerData")["profileimage"]["url"];
        });
      }
  ...
  _acceptDelivery() async {
    if (constantValues.estimatedDeliveryDay != "") {
      ...
      return ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("Please, select an estimated day duration!")));
    }
  }
